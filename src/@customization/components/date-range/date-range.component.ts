...
@Component({
    ...
    exportAs     : 'customizationDateRange',
    providers    : [
        {
            provide    : NG_VALUE_ACCESSOR,
            useExisting: forwardRef(() => CustomizationDateRangeComponent),
            multi      : true
        }
    ]
})
export class CustomizationDateRangeComponent implements ControlValueAccessor, OnInit, OnDestroy
{
    ...
    @HostBinding('class.customization-date-range') private _defaultClassNames = true;
    ...
    openPickerPanel(): void
    {
        ...
        const overlayRef = this._overlay.create({
            panelClass      : 'customization-date-range-panel',
            backdropClass   : '',
            hasBackdrop     : true,
    ...
    dateClass(): any
    {
        return (date: Moment): MatCalendarCellCssClasses => {
            ...
            {
                return ['customization-date-range', 'customization-date-range-start', 'customization-date-range-end'];
            }

            // If the date is the start date...
            if ( date.isSame(this._range.start, 'day') )
            {
                return ['customization-date-range', 'customization-date-range-start'];
            }

            // If the date is the end date...
            if ( date.isSame(this._range.end, 'day') )
            {
                return ['customization-date-range', 'customization-date-range-end'];
            }

            // If the date is in between start and end dates...
            if ( date.isBetween(this._range.start, this._range.end, 'day') )
            {
                return ['customization-date-range', 'customization-date-range-mid'];
            }
