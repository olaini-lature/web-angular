...
const generateVariableColors = (theme) =>
{
    ...
    }) =>
    {
        if ( opacityValue )
        {
            return `rgba(var(--customization-${name}-rgb), ${opacityValue})`;
        }
        if ( opacityVariable )
        {
            return `rgba(var(--customization-${name}-rgb), var(${opacityVariable}, 1))`;
        }
        return `rgb(var(--customization-${name}-rgb))`;
    };
...
// -----------------------------------------------------------------------------------------------------
// @ CUSTOMIZATION TailwindCSS Main Plugin
// -----------------------------------------------------------------------------------------------------
const theming = plugin.withOptions((options) => ({
    ...
    }) =>
    {
        ...
        const mapVariableColors = _.fromPairs(_.map(options.themes, (theme, themeName) => [
            ...
            ))), (value, key) => [[`--customization-${e(key)}`, value], [`--customization-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
        ]));
        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
        {
            ...
            const background = theme(`customization.customProps.background.${colorScheme}`);
            const foreground = theme(`customization.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--customization-${e(key)}`, value], [`--customization-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--customization-${e(key)}`, value], [`--customization-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
                }
            };
        });
