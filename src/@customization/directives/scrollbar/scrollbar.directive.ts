...
import { ScrollbarGeometry, ScrollbarPosition } from '@customization/directives/scrollbar/scrollbar.types';

/**
 * Wrapper directive for the Perfect Scrollbar: https://github.com/mdbootstrap/perfect-scrollbar
 */
@Directive({
    selector: '[customizationScrollbar]',
    exportAs: 'customizationScrollbar'
})
export class CustomizationScrollbarDirective implements OnChanges, OnInit, OnDestroy
{
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_customizationScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    @Input() customizationScrollbar: boolean = true;
    @Input() customizationScrollbarOptions: PerfectScrollbar.Options;
    ...
    {
        // Enabled
        if ( 'customizationScrollbar' in changes )
        {
            // Interpret empty string as 'true'
            this.customizationScrollbar = coerceBooleanProperty(changes.customizationScrollbar.currentValue);

            // If enabled, init the directive
            if ( this.customizationScrollbar )
            {
                this._init();
            }
        ...
        // Scrollbar options
        if ( 'customizationScrollbarOptions' in changes )
        {
            // Merge the options
            this._options = merge({}, this._options, changes.customizationScrollbarOptions.currentValue);
    ...
    isEnabled(): boolean
    {
        return this.customizationScrollbar;
    }
    ...
    private _init(): void
    {
        ...
        {
            this.customizationScrollbar = false;
            return;
        }
